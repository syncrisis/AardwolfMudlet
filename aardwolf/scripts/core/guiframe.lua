...
function GUIframe.addWindow(window, name, container, hideText)
    ...
    if not window then
        echo("[GUIframe] WARNING: addWindow called with nil window for '"..tostring(name).."'. Window may have been removed from code but still exists in save file. Skipping.\n")
        return
    end
...
function GUIframe.removeWindow(name, container)
    ...
    if not GUIframe.windows[name] then
        echo("[GUIframe] WARNING: removeWindow called for unknown window '"..tostring(name).."'. Skipping.\n")
        return
    end
    ...
    for con, wins in pairs(saveTbl.windows) do
        for _,name in ipairs(wins) do
            if type(name) == "string" then
                if GUIframe.windows[name] then
                    GUIframe.addWindow(GUIframe.windows[name], name, con)
                else
                    echo("[GUIframe] WARNING: Saved window '"..tostring(name).."' not found. It may have been removed. Run guisave to update.\n")
                end
            else
                local n, s = name[1], not name[2]
                if GUIframe.windows[n] then
                    GUIframe.addWindow(GUIframe.windows[n], n, con, s)
                else
                    echo("[GUIframe] WARNING: Saved window '"..tostring(n).."' not found. It may have been removed. Run guisave to update.\n")
                end
            end
        end
    end
